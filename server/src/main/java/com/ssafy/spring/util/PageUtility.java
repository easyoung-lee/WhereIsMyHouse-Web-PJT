...
public class PageUtility {
    ...
    public String getPageBar()    {
        ...
        if(((currentpagecount-1)/pagePercount*pagePercount)+1>beforetenpage)
            sb.append("<a  class='page-link'  href='javascript:pagelist("+ beforetenpage+")'>이전</a>");
        else
        	 sb.append("<a class='page-link .disabled'  href='#'>이전</a>");
        ...
        for (int i = firstpagecount; i < lastpagecount; i++)
        {
            ...
            if(i<=totalpagecount)
            {
                if (i == currentpagecount)
                    sb.append("<a class='page-link .active' href='#'>" + i + "</a>");
                else
                    sb.append("<a class='page-link' href='javascript:pagelist("+i+")'>" + i + "</a>");
